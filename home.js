...
function canWeDeliver(zipCode){
    return (deliveryAreaZipCodes.includes(zipCode) ? "You're in our delivery zone!"
                                                   : "Sorry, we can't deliver to that address.")
}
...
const canWeDeliverTwo = function(zipCode){
    const zipCheck = deliveryAreaZipCodes.forEach((zipcodeToCheck) => {if(zipcodeToCheck === zipCode){return true}});
    return (zipCheck === true) ? "You're in our delivery zone!" : "Sorry, we can't deliver to that address.";
}
...
{
    if (deals[i].title.includes("15% Off")){
        deals[i].title = deals[i].title.replace(deals[i].title, "10% Off!");
    }
}
